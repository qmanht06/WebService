export const INCREASE_QUANTITY = "cart/INCREASE_QUANTITY";
export const DECREASE_QUANTITY = "cart/DECREASE_QUANTITY";
export const ADD_TO_CART = "cart/ADD_TO_CART";
export const REMOVE_FROM_CART = "cart/REMOVE_FROM_CART";
export const FETCH_CART_LIST = "cart/FETCH_CART_LIST";
export const SET_CART_LIST = "cart/SET_CART_LIST";

export const SEARCH_FILTER_CHANGED = "filter/SEARCH_FILTER_CHANGED";
export const BRAND_FILTER_CHANGED = "filter/BRAND_FILTER_CHANGED";
export const SALE_FILTER_CHANGED = "filter/SALE_FILTER_CHANGED";

export const ADD_PRODUCT = "products/ADD_PRODUCT";
export const SET_PRODUCT_LIST = "products/SET_PRODUCT_LIST";
export const FETCH_PRODUCT_LIST = "products/FETCH_PRODUCT_LIST";
export const GET_SINGLE_PRODUCT = "products/GET_SINGLE_PRODUCT";
export const SET_SINGLE_PRODUCT = "products/SET_SINGLE_PRODUCT";
export const FETCH_ALL_PRODUCTS = "products/FETCH_ALL_PRODUCTS";
export const SET_ALL_PRODUCTS = "products/SET_ALL_PRODUCTS";
export const FETCH_PRODUCT_FOR_ORDER = "products/FETCH_PRODUCT_FOR_ORDER";
export const SET_PRODUCT_FOR_ORDER = "products/SET_PRODUCT_FOR_ORDER";

export const CHANGE_TOTAL_QUANTITY = "cart/CHANGE_TOTAL_QUANTITY";
